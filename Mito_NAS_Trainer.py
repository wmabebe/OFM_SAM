...
class Mito_NAS_Trainer:
    ...
    def eval(self,model,accumilator=None):
        mIoU = []
        model = model.eval()
        ...
        for inputs in self.test_dataloader: #enumerate(tqdm(self.test_dataloader, disable=self.no_verbose)): 
            ...
            outs = outs.squeeze().to(self.device)

            outs = torch.sigmoid(outs)
            outs = (outs > 0.5)

            gts = inputs["ground_truth_mask"].to(self.device)

            # print(f'outs.shape : {outs.shape}')
            # print(f'gts.shape : {gts.shape}')

            _, mious = compute_iou([o for o in outs],[gt for gt in gts])
            [mIoU.append(miou) for miou in mious]

            if accumilator != None:
                for idx in range(len(mious)):
                    img_idx = inputs["img_id"][idx]
                    pt, bx = inputs["points"][idx], inputs["boxes"][idx]
                    gt = gts[idx]
                    pred = outs[idx]                    
                    image = inputs["image"][idx]
                    accumilator[f'img-{img_idx}'] = (image,pt,bx,gt,pred,mious[idx])
        ...
        return average_mIoU, mIoU, accumilator
    ...
    def training_step(self, model, inputs):
        ...
        stk_gt = inputs["ground_truth_mask"] #torch.stack([gt for gt in inputs["ground_truth_masks"]], dim=0)
        stk_out = torch.stack([out for out in outputs.pred_masks], dim=0)
        stk_gt = stk_gt.squeeze(1).to(self.device)
        stk_out = stk_out.squeeze().to(self.device)
